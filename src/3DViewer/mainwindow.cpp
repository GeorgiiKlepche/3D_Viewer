...
MainWindow::MainWindow(QWidget *parent)
  ...
  if (dbManager.openDatabase()) {
      dbManager.displayDatabaseContent(ui->label);
      dbManager.closeDatabase();
  }
  connect(ui->button_vopros, SIGNAL(pressed()), this, SLOT(About()));
    ...
    if (ui->widget->Allocate()) {
      ...
      if (dbManager.openDatabase()) {
          if (dbManager.createTable()) {
              dbManager.insertFileInfo(ui->widget->findFile, ui->widget->data.count_of_vertexes, ui->widget->data.count_of_facets);
          } else {
